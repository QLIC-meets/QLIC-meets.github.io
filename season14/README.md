...
### Elena Callus
I will first introduce high harmonic generation in the context of non-classical states, which has motivated us to consider non-classicality and entanglement detection. I will then discuss the use of two copies of a state to derive witnesses of continuous-variable bipartite entanglement by means of interference. Specifically, one can access higher moments of the mode operators using a single beam splitter in order to detect non-Gaussian entanglement.

### Mauro D'Achille
I will present a new decomposition for the time evolution generated by a large class of field-theoretic quadratic Hamiltonians in terms of optical elements. The peculiarity of this decomposition consists in the way the time parameter is taken into account. Indeed, for such a class, it is always possible to decouple the time evolution in time-dependent phase shift transformations by means of a proper time-independent symplectic transformation composed by squeezers and beam splitters. I will conclude with physically relevant examples and applications aimed to analyze the spacetime structure of information for local and non-local field theories.

### Martin Gärttner
I will give an overview of the activities in the [Quantum Information and Quantum Many-Body Theory](https://mbqd.de/) group. This includes quantum thermalization in disordered spin systems, variational approaches to quantum many-body problems using neural networks as ansatz functions, entanglement detection and entropy estimation methods. A focus will be put on entanglement detection in continuous variable systems realized by ultracold atomic gases.
